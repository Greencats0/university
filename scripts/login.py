...
class csuf_portal_handler():
    ...
    def base_login(self):
        ...
        self.driver.find_element_by_css_selector(
            "input[id='username']").send_keys("{}".format(username))
        self.driver.find_element_by_css_selector(
            "input[id='password']").send_keys("{}".format(password))
        self.driver.find_element_by_css_selector(
    ...
    def get_recent_events(self):
        ...
        time.sleep(2)
        events = self.driver.find_elements_by_css_selector(
            "div[class='event']")
        for event in events:
            try:
                description = event.find_element_by_css_selector(
                    "div[class='description card-block calendar_event_due']").find_elements_by_tag_name("p")
                description_list = [element.text.strip()
                                    for element in description if element.text]
                description = "\n".join(description_list)
            except NoSuchElementException:
                description = ""

            event_title = event.find_element_by_css_selector(
                "h3[class='name d-inline-block']").text
            event_time = event.find_element_by_css_selector(
                "span[class='date pull-xs-right m-r-1']").text
            event_type = event.find_element_by_css_selector("div[class='d-inline-block mt-1 align-top']").find_element_by_css_selector(
                "img[class='icon ']").get_attribute("src").split("/")[-3].capitalize()
            ...
            if("Yesterday" not in event_time):
                print("="*75)
                print("[{}] {} @ {}".format(
                    event_type, event_title, event_time))
                print("Description: {}".format(description))
                print("="*75)

    def find_grades(self):
        self.driver.get(self.grades_url)
        grade_view = self.driver.find_elements_by_xpath(
            "//*[starts-with(@id, 'grade-report-overview-93391')]")
        mapping = {}
        for grade in grade_view:
            ...
            if(grade.text and len(string_split) >= 6):
                ...
                mapping[class_name] = grade.find_element_by_tag_name(
        ...
        for key, value in mapping.items():
            ...
            try:
                grade = self.driver.find_element_by_css_selector(
                    "td[class='level1 levelodd oddd1 baggb itemcenter  column-percentage']").text.replace("%", "").strip()
            except NoSuchElementException:
                grade = self.driver.find_element_by_css_selector(
                    "td[class='level1 levelodd oddd1 baggb itemcenter  column-grade']").text.replace("%", "").strip()
                ...
                # print(total_range)
            try:
                course_name = self.driver.find_element_by_css_selector(
                    "td[id='level1 levelodd oddd1 b2t column-itemname']").text
            except NoSuchElementException:
                course_name = None
            if(course_name is not None):
                if("CPSC-311" in course_name):
                    current_total = int(self.driver.find_element_by_css_selector(
                        "td[class='level1 levelodd oddd1 baggb itemcenter  column-grade']").text)
                    total_range = int(self.driver.find_element_by_css_selector(
                        "td[class='level1 levelodd oddd1 baggb itemcenter  column-range']").text)
        ...
        sorted_dictionary = sorted(
            grade_mapper.items(), key=operator.itemgetter(1), reverse=True)
    ...
    def get_grade_color(self, grade_amount: int):
        if(grade_amount >= 90 or grade_amount == 100):
            return "green"
        elif(grade_amount >= 80 and grade_amount <= 89):
            return "blue"
        elif(grade_amount >= 70 and grade_amount <= 79):
            return "yellow"
        elif(grade_amount >= 60 and grade_amount <= 69):
            return "magenta"
        else:
            return "red"
    ...
    def run_audit(self):
        ...
        self.driver.find_element_by_xpath(
            "//select[@id='reportType']/option[text()='HTML']").click()
        self.driver.find_element_by_css_selector(
            "input[title='Run Audit']").click()
        time.sleep(10)
    ...
    def automatic_schedule(self):
        self.driver.get("https://my.fullerton.edu/Portal/Dashboard/Psoft/StudentCenter/")
        time.sleep(7)
        greeting = self.driver.find_element_by_css_selector("span[class='PATRANSACTIONTITLE']")
        print(greeting.text)
